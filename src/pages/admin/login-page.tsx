...
const AdminLoginPage = () => {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const result = await api<{ session: { accessToken: string } }>("/api/auth/sign-in", {
        method: "POST",
      ...
      <PageTransition>
        <form onSubmit={submit} className="surface-card w-full max-w-md rounded-4xl p-8">
          ...
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            Ingresa con tu cuenta para crear y presentar tus experiencias.
          </p>
          <Input
            type="email"
            value={email}
          ...
          <Input
            ...
            required
          />
          <Button className="mt-6 w-full" isLoading={loading}>
            Ingresar
          </Button>
        </form>
      </PageTransition>
